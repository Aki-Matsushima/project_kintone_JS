    ...
    (event) => {
      ...
      button.onclick = async function () {
        try {
          ...
          OutputCheckBox(respRecord, record, "クラウドメンテナンス設計");
          ...
          for (let i = 0; i < respRecord.AWSリソース.value.length; i++) {
            resouCount +=
              respRecord.AWSリソース.value[i].value.AWSリソース_合計.value;
          }
          if (resouCount > 0) {
          ...
          //障害ランク表が新規アクセスの場合、お見積り条件に内容を追加する
          if (respRecord.障害ランク表.value === `新規作成`) {
            record[`レポトン用_お見積り条件_制約条件_新規1隠し`].value =
              "本案件開始時にサービスレベル（シビアリティランク）のご提示をお願いします。";
            record[`レポトン用_お見積り条件_制約条件_新規2隠し`].value =
...
function EstimatedCalc(respRecord, record, tableCode) {
  ...
  for (let i = 0; i < record[tableCode].value.length; i++) {
    ...
    if (
      ...
      if (
        ...
        record[tableCode].value[i].value.見積費用明細_メンバ1.value =
          ...
          record.オプション監視.value.length * 1.0 +
    ...
    if (
      record[tableCode].value[i].value.見積費用明細_項目.value === "AMS構築"
    ) {
      if (
        record[tableCode].value[i].value.見積費用明細_削除フラグ.value === "1"
      ) {
    ...
    if (
      record[tableCode].value[i].value.見積費用明細_項目.value ===
      "検証環境_インフラ監視(NewRelic、NewRelic Synthetics、AMS)"
    ) {
      if (
        record[tableCode].value[i].value.見積費用明細_削除フラグ.value === "1"
      ) {
    ...
    if (
      record[tableCode].value[i].value.見積費用明細_項目.value ===
      "検証環境_インフラ監視(NewRelic Synthetics)"
    ) {
      if (
        record[tableCode].value[i].value.見積費用明細_削除フラグ.value === "1"
      ) {
    ...
    if (
      record[tableCode].value[i].value.見積費用明細_項目.value ===
      "号口環境_インフラ監視(NewRelic Synthetics)"
    ) {
      if (
        record[tableCode].value[i].value.見積費用明細_削除フラグ.value === "1"
      ) {
...
//ヒアリングシートの監視ツールによって、運用基盤構築の文字列を変える関数
function Infrastructure(respRecord, record, systemNumber) {
  const infrastructureEnv = respRecord[`インフラ環境${systemNumber}`].value;
  const monitoringTool = respRecord[`監視ツール${systemNumber}`].value;
  const otherToolName =
    respRecord[`その他の場合はツール名を記載ください${systemNumber}`].value;

  if (infrastructureEnv === "新規") {
    switch (monitoringTool) {
      case "Datadog":
        record[`運用基盤構築${systemNumber}`].value =
          "Datadog/PD/AMS/Backlog/RVの設定を行います。";
        break;
      case "Newrelic":
        record[`運用基盤構築${systemNumber}`].value =
          "New Relic/PD/AMS/Backlog/RVの設定を行います。";
        break;
      case "その他":
        record[
          `運用基盤構築${systemNumber}`
        ].value = `${otherToolName}の設定を行います。`;
        break;
      case "新規":
        break;
    }
...
function DeleteFlag(respRecord, record, tableCode) {
  for (let i = 0; i < record[tableCode].value.length; i++) {
    ...
    if (
      ...
      if (
        record.設計_クラウドメンテナンス設計.value[0] !==
        "クラウドメンテナンス設計"
      ) {
        Disabled(record, tableCode, i, true);
        record[tableCode].value[i].value.見積費用明細_削除フラグ.value = "1";
    ...
    if (
      ...
      if (
        respRecord.インフラ環境1.value !== "新規" &&
        respRecord.インフラ環境2.value !== "新規" &&
        respRecord.インフラ環境3.value !== "新規"
      ) {
        Disabled(record, tableCode, i, true);
        record[tableCode].value[i].value.見積費用明細_削除フラグ.value = "1";
      } else {
